...
(
	...
	'cogenerating set',
	FALSE,
	'Assume that there is a cogenerating set $S$. We may assume $0 \notin S$. Take an infinite field $F$ which admits a commutative $R$-algebra structure, is larger than all the algebras in $S$ and admits a non-trivial automorphism (for example, a field of rational functions over some residue field of $R$). Then there is no homomorphism $F \to Q$ for $Q \in S$. Since $S$ cogenerates, this implies that all homomorphisms $F \to F$ are equal, a contradiction.'
),
(
	'CAlg(R)',
	'skeletal',
