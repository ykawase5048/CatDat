...
(
	...
	'locally small',
	TRUE,
	'There is a forgetful functor $\mathbf{Set}_\f \to \mathbf{Set}$ and $\mathbf{Set}$ is locally small.'
),
(
	'Set_f',
	'generator',
	TRUE,
	'The singleton set (which is <i>not</i> terminal) is a generator as it represents the forgetful functor $\mathbf{Set}_\f \to \mathbf{Set}$.'
),
(
	'Set_f',
	'cogenerator',
	TRUE,
	'The set $\{0,1\}$ is a cogenerator in $\mathbf{Set}_\f$: Assume that $f,g : X \rightrightarrows Y$ are two finite-to-one maps such that $h \circ f = h \circ g$ for all finite-to-one maps $h : Y \to \{0,1\}$. This exactly means $f^*(A)=g^*(A)$ for all finite subsets $A \subseteq Y$. Applying this to $A = \{f(x)\}$ for $x \in X$ we get $x \in f^*(\{f(x)\}) = g^*(\{f(x)\})$, so that $g(x) = f(x)$.'
),
(
	'Set_f',
	'extensive',
...
(
	...
	TRUE,
	'A congruence on a set $X$ in $\mathbf{Set}_\f$ is the same as an equivalence relation $R$ on $X$ whose equivalence classes are finite. In that case, the usual quotient map $p : X \to X/R$ is finite-to-one. Moreover, if $h : X/R \to Y$ is a map such that $h \circ p : X \to Y$ is finite-to-one, then $h$ is finite-to-one as well because $h^*(\{y\}) \subseteq p^*((h \circ p)^*(\{y\}))$ for all $y \in Y$. Therefore, $p$ is also the quotient in $\mathbf{Set}_\f$.'
),
(
	'Set_f',
	'locally cartesian closed',
	TRUE,
	'If $X$ is a set, the equivalence $\mathbf{Set}/X \simeq \mathbf{Set}^X$, $f \mapsto (f^*(\{x\}))_{x \in X}$ restricts to an equivalence $\mathbf{Set}_\f / X \simeq \mathbf{FinSet}^X$. This category is cartesian closed since <a href="/category/FinSet">the category $\mathbf{FinSet}$</a> is cartesian closed and products of cartesian closed categories are cartesian closed.'
),
(
	'Set_f',
	'mono-regular',
	TRUE,
	'If $f : X \to Y$ is a monomorphism, i.e. an injective map, it is an equalizer of the two maps $i_1, i_2 : Y \rightrightarrows Y \sqcup_X Y$ in $\mathbf{Set}$. But $i_1,i_2$ are injective, hence finite-to-one, so that $f$ is also an equalizer in $\mathbf{Set}_\f$.'
),
(
	'Set_f',
	'epi-regular',
	TRUE,
	'If $f : X \to Y$ is an epimorphism in $\mathbf{Set}_\f$, i.e. a surjective finite-to-one map, it is a coequalizer of the two maps $p_1, p_2 : X \times_Y Y \rightrightarrows Y$ in $\mathbf{Set}$. These maps are finite-to-one since $p_i^*(\{y\}) \cong f^*(\{y\})$ for $i=1,2$, and their coequalizer is also $f$ in $\mathbf{Set}_\f$: It suffices to observe that if $h : Y \to T$ is a map such that $h \circ f$ is finite-to-one, then $h$ is finite-to-one as well. In fact, surjectivity of $f$ implies $h^*(\{t\}) = f_*((h \circ f)^*(\{t\}))$ for $t \in T$.' 
),
(
	'Set_f',
...
(
	...
	'sequential limits',
	FALSE,
	'Consider the set $[n] := \{0,\dotsc,n\}$ for $n \in \IN$. The forgetful functor to $\mathbf{Set}$ is representable (by the singleton set), hence preserves all limits. Thus, if the diagram of truncation maps $\cdots \twoheadrightarrow [2] \twoheadrightarrow [1] \twoheadrightarrow [0]$ has a limit in $\mathbf{Set}_\f$, its underlying set is isomorphic to the limit taken in $\mathbf{Set}$, which is $\IN \cup \{\infty\}$. But there is no finite-to-one map $\IN \cup \{\infty\} \to [0]$.'
),
(
	'Set_f',
	'multi-cocomplete',
