...
(
	...
	'sifted colimits',
	TRUE,
	'We work with the representation of the category as $\mathbf{Vect}^{\leq 1}_{\IF_2}$, the category of vector spaces over $\IF_2$ of dimension $\leq 1$. It suffices to show that it is closed under sifted colimits in $\mathbf{Vect}_{\IF_2}$. More generally, we show this for $\mathbf{Vect}^{\leq d}_K \subseteq \mathbf{Vect}_K$, where $d \in \IN$ and $K$ is a field. So let  $X : \mathcal{I} \to \mathbf{Vect}_K$ be a sifted diagram with colimit $(u_i : X_i \to X_\infty)_{i \in \mathcal{I}}$. Since $\mathcal{I}$ is sifted, for finitely many objects $i_1,\dotsc,i_n \in \mathcal{I}$ there is an object $k$ that admits morphisms $i_1 \to k, \dotsc, i_n \to k$; this is all we need to know about $\mathcal{I}$. Assume that each $X_i$ is of dimension $\leq d$, we need to show this for $X_\infty$ as well.<br>
